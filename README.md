...
This starts SQL Server and runs the full Bronze to Gold pipeline automatically. Connect SSMS to `localhost,1433` to explore the Gold views.
...
On every push and pull request to `main`, the pipeline runs `pytest` on the churn model. A model that drops below its accuracy threshold will fail the build before it reaches main.
